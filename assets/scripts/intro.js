...
if (schemaTag) {
  try {
    ...
    const html = `<div id="postInfo"><div>${imgHtml}<div><a href="${authorUrl}" rel="author" target="_blank">${author}</a><br><time datetime="${pubISO}">${pubLong}</time>${editSpan}</div></div><button id="share" onclick="share()">Share 🔁</button></div>`;
