...
jquery__WEBPACK_IMPORTED_MODULE_0___default().extend((jquery__WEBPACK_IMPORTED_MODULE_0___default().summernote.lang), {
  'en-US': {
    ...
    video: {
      ...
      insert: 'Insert Video',
      url: 'Video URL',
      providers: '(YouTube, Google Drive, Vimeo, DailyMotion, Instagram)'
    },
    link: {
      link: 'Link',
      insert: 'Insert Link',
  ...
  VideoDialog_createClass(VideoDialog, [{
    ...
    value: function createVideoNode(url) {
      ...
      if (ytMatch && ytMatch[1].length === 11) {
        ...
        $video = external_jQuery_default()('<iframe>').attr('frameborder', 0).attr('src', 'https://www.youtube.com/embed/' + youtubeId + (start > 0 ? '?start=' + start : '')).attr('width', '640').attr('height', '360');
      } else if (gdMatch && gdMatch[0].length) {
        $video = external_jQuery_default()('<iframe>').attr('frameborder', 0).attr('src', 'https://drive.google.com/file/d/' + gdMatch[1] + '/preview').attr('width', '640').attr('height', '480').attr('allowfullscreen', 'allowfullscreen');
        ...
        $video = external_jQuery_default()('<iframe>').attr('frameborder', 0).attr('src', 'https://www.dailymotion.com/embed/video/' + dmMatch[2]).attr('width', '640').attr('height', '360');
...
(external_jQuery_default()).summernote = external_jQuery_default().extend((external_jQuery_default()).summernote, {
  ...
  options: {
    ...
    codeviewIframeWhitelistSrcBase: ['www.youtube.com', 'www.youtube-nocookie.com', 'gleev.xyz', 'www.facebook.com', 'vine.co', 'instagram.com', 'player.vimeo.com', 'www.dailymotion.com', 'player.youku.com', 'jumpingbean.tv', 'v.qq.com'],
    keyMap: {
      pc: {
        'ESC': 'escape',
...
var summernote_lite_button = renderer.create('<button type="button" class="note-btn theme-filter" tabindex="-1"></button>', function ($node, options) {
  // set button type
...
var dialog = renderer.create('<div class="note-modal" aria-hidden="false" tabindex="-1" role="dialog"></div>', function ($node, options) {
  ...
  $node.html(['<div class="note-modal-content">', options.title ? '<div class="note-modal-header"><div aria-label="Close" class="close"></div><h4 class="note-modal-title">' + options.title + '</h4></div>' : '', '<div class="note-modal-body">' + options.body + '</div>', options.footer ? '<div class="note-modal-footer">' + options.footer + '</div>' : '', '</div>'].join(''));
